...
│   │   ├── menu_migracao_vhd.sh               # Migração de discos (VHD, VHDX, VMDK, VDI)
...
Por padrão, o `main.sh` verifica por atualizações no GitHub a cada execução.

### Ativar / Desativar Atualizações Automáticas
Para ambientes corporativos que exigem versões fixas por política de segurança:
- Acesse: **Menu 3 (Extras) → 7 (Central de Gerenciamento) → 3 ➜ Configuração de Atualização Automática**
- **Opção 1:** Desativar Atualização Automática (bloqueia downloads ao iniciar)
- **Opção 2:** Ativar Atualização Automática (restaura comportamento padrão)
